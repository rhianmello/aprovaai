-- Rollback retrospectivo do lote autoral de Portugues adicional Transpetro.
-- Lote: transpetro_portuguese_additional_zero_coverage_v1
-- Criado em: 2026-09-18
--
-- Este rollback remove somente os registros criados para o lote autoral
-- transpetro_portuguese_additional_zero_coverage_v1. Ele preserva Portugues-base,
-- questoes QAPI, questoes historicas, cursos, preparacoes, usuarios, pagamentos,
-- matriculas e autenticacao.
--
...
where source_type = 'AUTORAL'
  and source_reference like 'transpetro_portuguese_additional_zero_coverage_v1:%'
...
where qac.applicability_id = qa.id
  ...
  and q.source_reference like 'transpetro_portuguese_additional_zero_coverage_v1:%';
...
where r.question_id = q.id
  and q.source_type = 'AUTORAL'
  and q.source_reference like 'transpetro_portuguese_additional_zero_coverage_v1:%'
  and r.rule_version = 'transpetro_portuguese_additional_zero_coverage_v1';
...
select
  (select count(*) from public.questions
   where source_type = 'AUTORAL'
     and source_reference like 'transpetro_portuguese_additional_zero_coverage_v1:%') as remaining_questions,
  (select count(*) from public.question_preparation_editorial_reviews
   where rule_version = 'transpetro_portuguese_additional_zero_coverage_v1') as remaining_reviews;
