...
Deno.serve(async(req)=>{
  ...
  if(telefone.length<8)return json({message:'Informe um WhatsApp/celular válido.'},400)
  ...
  if(existing?.user)
    return json({message:'Este e-mail já possui cadastro. Clique em “Já tenho cadastro” e entre na sua conta para continuar.'},409)
  ...
  if(createError||!created.user){
    const msg=(createError?.message||'Não foi possível criar sua conta.').toLowerCase().includes('already')
  ...
  const{data:purchase,error:purchaseError}=await admin
    ...
    .insert({
      ...
      metadata:{payment_flow:'checkout_pro_monthly_public'}
    })
    .select('id')
    .single()
  ...
  if(!mp.ok||!mpData.id||!mpData.init_point){
    ...
    return json({message:'Sua conta foi criada, mas o Mercado Pago não conseguiu iniciar o checkout. Entre na conta e tente novamente.'},502)
  }

  const{error:updateError}=await admin.from('purchases').update({
    metadata:{
      payment_flow:'checkout_pro_monthly_public',
