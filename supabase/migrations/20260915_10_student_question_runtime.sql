...
begin
  ...
  select
    ...
    coalesce(content.content_title, 'Questões do edital'),
    coalesce(m.subtopic, ''),
  ...
  where a.preparation_id = p_preparation_id
    and a.active = true;
  -- FASE 3: adicionar aqui, em um único ponto, a exigência
  -- ADERENTE + APROVADA da revisão editorial por preparação.
end;
$$;
...
comment on function public.load_student_question_bank(bigint, uuid) is
  'Carregamento central e autorizado do banco de questões do aluno; trava editorial será adicionada na Fase 3.';
