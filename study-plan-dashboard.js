...
async function render(){
 if(mounted)return;const host=document.querySelector('#home .dashboard-grid')||document.querySelector('#home');if(!host||!window.__NP_AUTH_USER||!slug)return;
 ...
 const todaySessions=sess.filter(s=>new Date(s.started_at)>=today&&new Date(s.started_at)<tom&&s.status==='completed').length,plannedSessions=activities.filter(a=>a.day_of_week===today.getDay()).length;
 ...
 card.innerHTML=`<div class="np-plan-head"><div><h2>📚 Meu Plano</h2><div class="np-plan-name">${esc(plan.name)}</div></div><a class="btn secondary" href="./meu-plano.html?course=${encodeURIComponent(slug)}">Continuar plano</a></div><div class="np-plan-kpis"><div><span>Planejado</span><b>${fmt(plannedWeek)}</b><small>esta semana</small></div><div><span>Estudado</span><b>${fmt(studiedWeek)}</b><small>esta semana</small></div><div><span>Progresso</span><b>${pct}%</b><small>${todaySessions}/${plannedSessions} sessões hoje</small></div></div><div class="np-progress"><div><span>Meta semanal</span><strong>${fmt(studiedWeek)} / ${fmt(plannedWeek)}</strong></div><div class="np-progress-track"><i style="width:${pct}%"></i></div><p>${message}</p></div><div class="np-plan-today"><div><strong>Hoje</strong><span>${fmt(studiedToday)} estudados · ${fmt(Math.max(0,plannedToday-studiedToday))} restantes</span></div><div class="np-day-bars">${byDay.map((v,i)=>`<div class="np-day-bar"><span>${fmt(v)}</span><i><b style="height:${Math.max(3,Math.round(v/max*100))}%"></b></i><small>${days[i]}</small></div>`).join('')}</div></div>`;
 host.appendChild(card);mounted=true;
}
function boot(){render().catch(e=>console.error('[Meu Plano Dashboard]',e))}const obs=new MutationObserver(()=>{if(!mounted)boot()});obs.observe(document.body,{childList:true,subtree:true});boot();
})();
