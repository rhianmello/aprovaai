...
function makePlan(items,prepName){
 const by={};(items||[]).forEach(x=>{const s=x.academic_subjects?.name||'Conhecimentos Específicos';(by[s]??=[]).push(x)});
 const subjects=Object.keys(by);const days=['SEG','TER','QUA','QUI','SEX','SAB'];
 const plan=[{day:'DOM',title:'Simulado + revisão da semana',q:70,simulado:true,topics:['Simulado completo','Revisão dos erros','Diagnóstico por disciplina']}];
 days.forEach((day,i)=>{const names=subjects.length?subjects.filter((_,j)=>j%days.length===i):[];const fallback=subjects.length?[subjects[i%subjects.length]]:[];const use=names.length?names:fallback;const topics=[];use.forEach(s=>(by[s]||[]).slice(0,8).forEach(x=>topics.push(x.title)));plan.push({day,title:use.length?use.join(' + '):'Estudo dirigido',q:0,keywords:use,topics:topics.length?topics:['Conteúdo do edital']})});
 return plan;
}
async function configureDashboard(client,course,user){
 ...
 if(le)throw le;
 const prep=(links||[])[0]?.preparations;
 if(!prep)throw new Error('Preparação acadêmica não vinculada ao curso.');
 const {data:items,error:ie}=await client.from('academic_content_items').select('id,title,subject_id,topic_id,academic_subjects(name)').eq('preparation_id',prep.id).eq('active',true).order('sort_order');
 if(ie)throw ie;
 const subjectName=x=>x?.academic_subjects?.name||'Conhecimentos Específicos';
 const bankLoader=async()=>{
   const [qa,qac,q]=await Promise.all([
     client.from('question_applicability').select('id,question_id').eq('preparation_id',prep.id).eq('active',true),
     client.from('question_applicability_content').select('applicability_id,content_item_id').eq('preparation_id',prep.id),
     client.from('questions').select('id,statement,alternatives,answer,explanation,source_reference,year,is_original').eq('active',true)
   ]);
   if(qa.error||qac.error||q.error)throw qa.error||qac.error||q.error;
   const qmap=new Map((q.data||[]).map(x=>[x.id,x]));
   const cmap=new Map((items||[]).map(x=>[x.id,x]));
   const linksBy=new Map();(qac.data||[]).forEach(x=>{if(!linksBy.has(x.applicability_id))linksBy.set(x.applicability_id,[]);linksBy.get(x.applicability_id).push(x.content_item_id)});
   return (qa.data||[]).map(a=>{const q=qmap.get(a.question_id);if(!q)return null;const cs=(linksBy.get(a.id)||[]).map(id=>cmap.get(id)).filter(Boolean);const c=cs[0];const alts=Array.isArray(q.alternatives)?q.alternatives:Object.entries(q.alternatives||{}).map(([letra,texto])=>({letra,texto}));return {id:q.id,enunciado:q.statement,alternativas:Object.fromEntries(alts.map(a=>[a.letra,a.texto])),gabarito:q.answer,explicacao:q.explanation||'',disciplina:subjectName(c),assunto:c?.title||'Questões do edital',subassunto:'',fonte:q.source_reference||''}}).filter(Boolean);
 };
 const pos=prep.academic_positions?.name||course.name.replace(/^Transpetro 2026 — /,'');
 const edition=prep.academic_editions||{};
 const qLabel=edition.code?`Edital ${edition.code}/2026`:'';
 window.STUDY_CONFIG={
   slug:course.slug,courseSlug:course.slug,title:pos,shortTitle:`Transpetro 2026 • ${pos}`,badge:`TRANSPETRO 2026 • ${pos.toUpperCase()}`,subtitle:`${qLabel} • ${edition.banca||'Fundação Cesgranrio'}`,
   backUrl:'minha-conta.html',videoQuery:`Transpetro 2026 ${pos} Cesgranrio`,historyKey:`aprovaai_${course.slug}_history_v1`,description:`Preparação específica para ${pos}, baseada no edital do processo seletivo público da Transpetro 2026.`,bankLoader,group:q=>{const s=norm([q.disciplina,q.assunto,q.subassunto,q.enunciado].join(' '));if(s.includes('portugues'))return'Português';if(s.includes('ingles'))return'Inglês';return q.disciplina||'Específicos'},plan:makePlan(items,prep.name),presets:[]
 };
}
...
(async()=>{try{
 ...
 await configureDashboard(client,course,session.user);window.__NP_AUTH_USER=session.user;window.__NP_COURSE=course;
