...
const load=()=>new Promise((ok,bad)=>{if(window.supabase?.createClient)return ok(window.supabase);const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';s.async=true;let done=false;const finish=(fn,v)=>{if(done)return;done=true;fn(v)};s.onload=()=>window.supabase?.createClient?finish(ok,window.supabase):finish(bad,Error('Supabase não carregou'));s.onerror=()=>finish(bad,Error('Falha ao carregar Supabase'));setTimeout(()=>finish(bad,Error('timeout ao carregar Supabase')),10000)});
...
/* A sessão já foi confirmada no login. getSession é local e evita travar esperando uma ida desnecessária ao servidor. */
...
/* Primeiro consultamos a própria matrícula. Isso evita depender de RPC para a abertura do curso. */
...
/* Se a consulta direta falhar, tenta a função de acesso como fallback. */
...
/* Registro de dispositivo nunca pode impedir o acesso ao curso. */
