...
function setup(){
  ...
  const findCourse=()=>{
    try{return (window.courses||[]).find(c=>c.slug===slug)}catch(e){return null}
  };
  const update=()=>{
    const c=findCourse();
    if(!c||!choices.children.length||choices.querySelector('.empty'))return false;
    const name=c.name||slug;
    box.innerHTML='<div><div style="font-size:11px;color:#8be3b7;font-weight:900;text-transform:uppercase">Teste grátis selecionado</div><h2 style="margin:6px 0 4px;font-size:21px">'+esc(name)+'</h2><div style="font-size:12px;color:#aab2bd">Você chegou aqui pelo botão deste curso. O teste começa logo abaixo.</div></div><button id="change-course" class="btn dark" type="button">🔎 Escolher outro curso</button>';
    ...
    if(b)b.onclick=()=>{choices.style.display='grid';if(toolbar)toolbar.style.display='flex';box.remove();};
    return true;
  };
  const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));
  if(!update()){
    let n=0;const timer=setInterval(()=>{if(update()||++n>40)clearInterval(timer)},250);
  }
}
